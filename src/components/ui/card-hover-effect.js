      ...
      {items?.map((item, idx) => (
        <div
          ...
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 h-full w-full bg-sky-100 block rounded-3xl"
...
export const Card = ({
  ...
  return (
    (<div
      className={cn(
        "rounded-2xl h-full w-full p-4 overflow-hidden bg-white border border-sky-200 dark:border-white/[0.2] group-hover:border-sky-200 relative z-20",
        className
      )}>
